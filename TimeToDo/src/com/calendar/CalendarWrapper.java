...
public class CalendarWrapper {
	...
	public CalendarWrapper() {
		...
		_shortDayNames = new String[_calendar
				.getActualMaximum(Calendar.DAY_OF_WEEK)];
		_shortMonthNames = new String[_calendar
				.getActualMaximum(Calendar.MONTH) + 1]; // Months are 0-based so
														// size is Max + 1

		for (int i = 0; i < _shortDayNames.length; i++) {
			_shortDayNames[i] = DateUtils.getDayOfWeekString(i + 1,
					DateUtils.LENGTH_SHORT);
		}

		for (int i = 0; i < _shortMonthNames.length; i++) {
			_shortMonthNames[i] = DateUtils.getMonthString(i,
					DateUtils.LENGTH_SHORT);
		}
	}
